...
separated_FT = zeros(szX,szY,a_num,3,'gpuArray');
...
xc=gpuArray(floor(szX/2+1));% the x-coordinate of the center
yc=gpuArray(floor(szY/2+1));% the y-coordinate of the center
yr=gpuArray(Y-yc);
xr=gpuArray(X-xc);
...
for ii=1:a_num
 ...
 function [shiftvalue] = frequency_estimation(ft_im,suppress_noise_factor,fmask)
    ...
    norm_ft=zeros(size(ft_im),'gpuArray');
    im=norm_ft;
    re_f=im;
    reference=zeros(xsize,ysize,a_num,'gpuArray');
    for ii=1:a_num
        for jj=1:p_num
    ...
    shiftvalue=zeros(a_num,3,2,'gpuArray');
    for ii=1:a_num
        for jj=1:3
            if jj==1
