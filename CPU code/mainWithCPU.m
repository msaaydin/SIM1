...
anaZaman = tic;
tic;
raw = double(zeros(szX,szY,9));
orientation0    = double(zeros(szX,szY,3));
orientation60   = double(zeros(szX,szY,3));
orientation120  = double(zeros(szX,szY,3));
...
SIMcenter             = double(zeros([size(im_orig),3])); 
SIMplus               = double(zeros([size(im_orig),3]));
SIMmin                 = double(zeros([size(im_orig),3]));
%*************************************************************************
...
orientation120(:,:,3)  = fftshift(fft2(raw(:,:,9)));
 ...
 noiseimagef=zeros(szX,szY,angle_num,phase_num);
...
phaseTime = tic;
    ...
    % =====================orientation = 0 phase estimation with phase of peak  ====================== 
            for ii = 1:3                
                 phs_peak(1,ii) = CalculatePhase(orientation0(:,:,ii),imFTNormSize);                
            end 
%             wait(gd);
%             disp('-----------------------------------------------------------------');
    % =====================orientation = 60 ======================           
            for ii = 1:3
                phs_peak(2,ii) = CalculatePhase(orientation60(:,:,ii),imFTNormSize);
            end
%             wait(gd);
%             disp('-----------------------------------------------------------------');
    % =====================orientation = 120 ======================          
            for ii = 1:3
                phs_peak(3,ii) = CalculatePhase(orientation120(:,:,ii),imFTNormSize);
            end  
%             wait(gd);
%             disp('-----------------------------------------------------------------');
...
objCalc = tic;
% ObjP
ObjP = EstimateObjP(fCent,OTFo,cuOtf);
...
imwrite(uint16((DsumA)),'reconst_image.tif')
